...
def load_ollama_model():
    llm = Ollama(model=config["mistral"])
    return llm
...
def load_retrieval_chain(llm, vector_db):
    return RetrievalQA.from_llm(llm=llm, retriever=vector_db.as_retriever(search_kwargs={"k": config["pdfs_folder_path"]["path"]}), verbose=True)

def create_pdf_chat_runnable(llm, vector_db, prompt):
    runnable = (
        {
        "context": itemgetter("human_input") | vector_db.as_retriever(search_kwargs={"k": config["pdfs_folder_path"]["path"]}),
